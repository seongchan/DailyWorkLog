...
- `SettingTab` still uses the imperative `display()` method, not the newer declarative `getSettingDefinitions()` API (Obsidian 1.13.0+). This is a deliberate, accepted trade-off: adopting it would mean either bumping `minAppVersion` to 1.13.0, or maintaining two parallel settings definitions in sync (Obsidian's own migration guide requires updating both every time a setting changes) — not worth it for two dropdowns. `eslint-plugin-obsidianmd`'s `settings-tab/prefer-setting-definitions` warning on this is expected and should stay unaddressed unless the settings surface grows significantly.
